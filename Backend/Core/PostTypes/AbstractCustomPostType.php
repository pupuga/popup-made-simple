...
abstract class AbstractCustomPostType
{
    ...
    public function setRewrite(string $slug, bool $withFront = false, bool $pages = true, bool $feeds = false): self
    {
        $this->args['rewrite'] = array('slug' => $slug, 'with_front' => $withFront, 'pages' => $pages, 'feeds' => $feeds);
        return $this;
    }
    ...
    public function setParamsForHiddenMode(): self
    {
        $this->setSupports(['title', 'editor', 'custom-fields'])
            ->setPublic(false)
            ->setPubliclyQueryable(false)
            ->setQueryVar(false)
            ->setHasArchive(false)
            ->setShowInAdminBar(false);
